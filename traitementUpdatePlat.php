...
if(empty($_FILES['image']['tmp_name'])) {
 ...
 $req = $bdd->prepare('UPDATE `restaurant`.`plat` SET `nom` = :nom, `prix` = :prix WHERE `plat`.`id` = :id');
 ...
 $req->execute(array(
   ...
   'id' => $idPlat
    ...
    if ($image_sizes[0] > $maxwidth OR $image_sizes[1] > $maxheight) $erreur = "Image trop grande";
     ...
     $req = $bdd->prepare('UPDATE `restaurant`.`plat` SET `nom` = :nom, `prix` = :prix, `image` = :image WHERE `plats`.`id` = :id_plat');
